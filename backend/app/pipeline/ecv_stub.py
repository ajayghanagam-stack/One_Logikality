...
async def _persist_findings(packet_id: uuid.UUID) -> None:
    ...
    async with SessionLocal() as session:
        ...
        # On failure we log and proceed with zero docs — the dashboard will
        # render an empty Documents tab rather than stale canned data.
        try:
    ...
    # have committed their rows. A failure here leaves sections
    # unwritten, which the dashboard endpoint reports as 409 / not ready.
    try:
